const cookieName = "booking_admin_invite_continuation";
const digestPattern = /^[0-9a-f]{64}$/;
const valuePattern = /^v1\.([0-9a-f]{64})\.([A-Za-z0-9_-]{43})$/;
const encoder = new TextEncoder();

/**
 * Create signed same-origin Admin Invite continuation capabilities.
 *
 * @param {string} secret Environment-owned authentication secret.
 * @returns {object} Issue, read, and clear cookie operations.
 */
export function createAdminInviteContinuation(secret) {
  if (typeof secret !== "string" || secret.length === 0) {
    throw new Error("Admin Invite continuation secret is missing.");
  }
  ...
  return {
    ...
    async issueCookie(request, tokenDigest) {
      if (!digestPattern.test(tokenDigest)) {
        throw new Error("Admin Invite continuation digest is invalid.");
      }
...
/** @returns {Promise<CryptoKey>} HMAC key separated from every other use. */
async function derivePurposeSigningKey(secret) {
  ...
  const keyBytes = await crypto.subtle.sign(
    "HMAC",
    baseKey,
    encoder.encode("booking-system:admin-invite-continuation:v1:key"),
  );
...
/** @returns {string | null} Exact Admin continuation cookie value. */

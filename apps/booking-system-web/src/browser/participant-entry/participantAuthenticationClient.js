import { createAuthClient } from "better-auth/react";

const participantAuthenticationClient = createAuthClient();

/**
 * Start Google sign-in with fixed Participant success and failure destinations.
 *
 * @returns {Promise<object | null>} Better Auth initiation data when no redirect occurs.
 */
export async function continueParticipantWithGoogle() {
  const result = await participantAuthenticationClient.signIn.social({
    provider: "google",
    callbackURL: "/",
    errorCallbackURL: "/api/auth/participant-error",
  });

  if (result.error !== null) {
    throw authenticationFailure();
  }

  return result.data;
}

/**
 * Terminate the current normal Better Auth session.
 *
 * @returns {Promise<object | null>} Better Auth sign-out data.
 */
export async function signOutParticipant() {
  const result = await participantAuthenticationClient.signOut();

  if (result.error !== null) {
    throw authenticationFailure();
  }

  return result.data;
}

/**
 * Create one language-neutral browser authentication failure.
 *
 * @returns {Error} The application failure.
 */
function authenticationFailure() {
  const error = new Error("authentication-failed");

  error.outcome = "authentication-failed";

  return error;
}

...
- [ ] The application pins the then-current stable compatible free MUI Core
...
- [ ] One repository-owned theme and `CssBaseline` define cohesive typography,
      spacing, surfaces, responsive breakpoints, and visible focus treatment
      without creating a separate UI/design-system package.
- [ ] The existing `/admin` states and transitions remain behaviorally
...
- [ ] The migrated flow uses familiar Material components and remains fully
...
- [ ] Automated axe-style Playwright scanning is integrated into the local
      ...
      keyboard assertions.
- [ ] MUI Core components are used directly where appropriate. No competing
...
remote Cloudflare work, and provider UI automation remain excluded. Create a
fresh code-grounded Markplane implementation plan immediately before work;
`plan` intentionally remains `null` now.
